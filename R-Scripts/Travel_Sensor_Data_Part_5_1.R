...
agg_bluetooth_data_normalized <- agg_bluetooth_data

# Gets the Max Value for Each Row for the new Normalized Dataframe.
agg_bluetooth_data_normalized$max <- apply(agg_bluetooth_data_normalized,1,max)
...
# Divides first column
  agg_bluetooth_data_normalized[,i] <- as.numeric(paste(agg_bluetooth_data_normalized[,i])) / as.numeric(as.character(agg_bluetooth_data_normalized$max))
# Add 1 to i
i = i + 1
}

# Removes the max column
agg_bluetooth_data_normalized$max <- NULL

### CREATES THE ARRAY FOR "agg_bluetooth_data" ###
...
### CREATES THE ARRAY FOR "agg_bluetooth_data_normalized" ###
# Creates an empty column to store the array.
agg_bluetooth_data_normalized$twentyfourhours_normalized <- NA
# Adds all of the values into column twentyfourhours with "," as the separator.
agg_bluetooth_data_normalized$twentyfourhours_normalized <- apply(agg_bluetooth_data_normalized[, 1:24], 1, paste, collapse=",")
# Remove all spaces.
agg_bluetooth_data_normalized$twentyfourhours_normalized <- gsub(" ", "", agg_bluetooth_data_normalized$twentyfourhours_normalized)
# Delete first 24 columns.
agg_bluetooth_data_normalized <- agg_bluetooth_data_normalized[ -c(1:24) ]
...
# Adds the normalized column to the dataframe.
AggBluetoothFinal$twentyfourhours_normalized <- agg_bluetooth_data_normalized$twentyfourhours_normalized
...
# Put brackets around values in column "twentyfourhours_normalized".
AggBluetoothFinal$twentyfourhours_normalized <- paste0("[",AggBluetoothFinal$twentyfourhours_normalized, "]")
...
write.csv(AggBluetoothFinal, file = "Sept12_AggTwentyFourHour_Normalized.csv",row.names=FALSE)
